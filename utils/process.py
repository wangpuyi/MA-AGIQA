...
import pandas as pd
import os
def dataset_imagereward(label_path, type):
    if type == 'train':
        path = os.path.join(label_path, 'train.csv')
    elif type == 'validation':
        path = os.path.join(label_path, 'validation.csv')
    else:
        path = os.path.join(label_path, 'test.csv')
    df = pd.read_csv(path)
    pic_paths = df['path'].tolist()
    scores = df['fidelity'].values.tolist()

    return pic_paths, scores

def dataset_agiqa(label_path, type):
    if type == 'train':
        path = os.path.join(label_path, 'train.csv')
    elif type == 'validation':
        path = os.path.join(label_path, 'validation.csv')
    else:
        path = os.path.join(label_path, 'data.csv')

    df = pd.read_csv(path)
    pic_paths = df['name'].tolist()
    scores = df['mos_quality'].values.tolist()

    return pic_paths, scores

def dataset_general(label_path, type, dataset, set_seed=None):
    '''
        return:
            pic_paths: list of picture names
            scores: list of scores
    '''
    if dataset == 'AGIQA_3k':
        csv_file = 'data.csv'
    elif dataset == 'AIGCQA_20k':
        csv_file = 'info_train.csv'
    ...
    if type == 'train':
        ...
        path = os.path.join(label_path, csv_file)
    elif type == 'random':
        path = os.path.join(label_path, csv_file)
        df = pd.read_csv(path)
        pic_paths = df['name'].tolist()
        if dataset == 'AGIQA_3k':
            ...
            scores = df['mos'].values.tolist()
        sel_num = list(range(0, len(pic_paths)))
        import random
        if set_seed:
            random.seed(set_seed)
        random.shuffle(sel_num)
        train_index = sel_num[:int(len(pic_paths) * 0.7)]
        val_index = sel_num[int(len(pic_paths) * 0.7): int(len(pic_paths) * 0.8)]
        test_index = sel_num[int(len(pic_paths) * 0.8):]

        train_pic_paths = [pic_paths[i] for i in train_index]
        train_scores = [scores[i] for i in train_index]
        val_pic_paths = [pic_paths[i] for i in val_index]
        val_scores = [scores[i] for i in val_index]
        test_pic_paths = [pic_paths[i] for i in test_index]
        test_scores = [scores[i] for i in test_index]

        return train_pic_paths, train_scores, val_pic_paths, val_scores, test_pic_paths, test_scores
...
def dataset_laion(label_path, type):
    if type == 'train':
        path = os.path.join(label_path, 'mytrain.csv')
    elif type == 'validation':
        path = os.path.join(label_path, 'myvalidation.csv')
    else:
        path = os.path.join(label_path, 'mytest.csv')
    df = pd.read_csv(path)
    pic_paths = df['path'].tolist()
    scores = df['rating'].values.tolist()
